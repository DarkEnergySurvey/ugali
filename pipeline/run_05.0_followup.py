...
def run(self):
    targets = numpy.genfromtxt(self.opts.targets,dtype=None)
    ...
    for target in targets:
        ...
        farm.submit_all(coords=coords,local=self.opts.local,debug=self.opts.dryrun)

Pipeline.run = run
pipeline = Pipeline(description,components)
pipeline.parser.add_argument('-t','--targets', default=None, required=True)
pipeline.parse_args()
pipeline.execute()
